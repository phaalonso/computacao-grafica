...
Experimento para demonstracao da projecao perspectiva
**/
void phDesenharEspiralX()
{

    float r = 5.0; // raio do espiral
    float t;       // angulo
    float x, y, z;
    ...
    for (t = -30; t <= 5; t += 0.01)
    {
        ...
        z = t;
        std::cout << "Posicionando em (x,y,z): " << x << ", " << y << ", " << z << std::endl;
        glVertex3f(z, y, -10 + x); // observar efeito do valor da variacao de t. Ex: -5 <= t <= -100
        // lembrar que na proj perspectiva z=0 esta na origem do observador, por isso, deslocar para zoom out
    }
    glEnd();
}
...
void phDesenharEspiralY()
{
    ...
    for (t = -30; t <= 5; t += 0.01)
    {
        ...
        z = t;
        std::cout << "Posicionando em (x,y,z): " << x << ", " << y << ", " << z << std::endl;
        glVertex3f(y, z, -10 + x); // observar efeito do valor da variacao de t. Ex: -5 <= t <= -100
        // lembrar que na proj perspectiva z=0 esta na origem do observador, por isso, deslocar para zoom out
    }
    glEnd();
}
